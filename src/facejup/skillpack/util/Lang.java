package facejup.skillpack.util;

public class Lang {
	
	public static final String tag = Chat.translate("&9&l(&bS&r&oM&9&l) &a");
	public static final String consoleUse = tag + Chat.translate("Only players can use this command");
	public static final String invalidArgs = tag + Chat.translate("Incorrect Syntax");
	public static final String nullSkill = tag + Chat.translate("You don't have that skill");
	public static final String nullPlayer = tag + Chat.translate("Player not found");
	public static final String nullShop = tag + Chat.translate("Shop not found");

}
